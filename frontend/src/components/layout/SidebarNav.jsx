...
import {
    ...
    Briefcase, User, Gamepad2, FilePenLine
} from 'lucide-react';
        ...
        <div className="flex flex-col h-full">
                ...
                <div className="flex items-center gap-3">
                    <div className="relative flex-shrink-0">
                        {user?.photo ? (
                            ...
                            <div className="w-11 h-11 rounded-full bg-gradient-to-br from-[#8b6dff] to-[#7657f6] flex items-center justify-center text-white font-bold text-sm">
                                {user?.first_name?.charAt(0) || user?.username?.charAt(0) || 'U'}
                            </div>
                        )}
                        <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-400 border-2 border-[#1b1635] rounded-full" />
                    </div>
                    <div className="min-w-0 flex-1">
                        <h3 className="text-sm font-bold text-white truncate">
                            {user ? `${user.first_name} ${user.last_name}` : 'Cargando...'}
                        </h3>
                        <p className="text-[11px] text-violet-200/75 font-medium flex items-center gap-1">
                            {effectiveRole && ROLE_META[effectiveRole] && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
                            {getRoleLabel()}
                            {allRoles.length > 1 && <span className="text-violet-400/60 font-normal">· {allRoles.length} roles</span>}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="px-4 pb-3">
                <button
                    ...
                    className="flex items-center gap-2 px-3 py-2 w-full rounded-xl text-xs font-semibold text-violet-100/80 bg-white/[0.08] hover:bg-white/[0.12] border border-white/10 transition-all"
                >
            ...
            <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                {['ADMIN', 'TEACHER', 'PRACTICE_TEACHER', 'STUDENT'].includes(effectiveRole) && (
                    <SidebarItem
                        icon={FilePenLine}
                        label={isStudent ? 'Mis actas y notas' : 'Actas y rúbricas'}
                        to="/records"
                        onClick={onClose}
                        subtitle={isStudent ? 'Firmas y notas' : 'Firmas y evaluación'}
                    />
                )}
                ...
                {isStudent && <>
                    ...
                    <button
                        ...
                        className="flex items-center gap-3 px-4 py-3 rounded-xl w-full text-violet-100/80 hover:bg-white/10 hover:text-white transition-all group mt-1"
                    >
                        <div className="w-5 h-5 rounded-full bg-[#7657f6] text-white flex items-center justify-center flex-shrink-0 group-hover:scale-110 transition-transform">
                            <Plus size={12} />
                        </div>
            ...
            <div className="px-4 py-4 border-t border-white/10">
                <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.08]">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-violet-200/75 text-[10px] truncate">Gestión Académica · UPN</p>
                    </div>
                    <button onClick={handleLogout} className="text-violet-200/75 hover:text-red-400 transition-colors p-1 rounded-lg hover:bg-red-400/10" title="Cerrar sesión">
